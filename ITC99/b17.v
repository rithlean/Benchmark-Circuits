    ...
    if(RESET == 1'b1) begin
		...
		case(State)
			...
			StateT1 : begin
				...
				// fs 062299
				TEMPORARY = rEIP / 4;
				Address <= TEMPORARY [29:0];  // fix: changed = to <=
				//Address <= (rEIP / 4) % (2 ** 30);
				BE_n <= ByteEnable;
				M_IO_n <= MemoryFetch;
...
// -----------------------------------------------------------------------------
// FIX 1: Address is now driven exclusively with blocking (=) throughout the
//         entire always block — including the reset branch. Synopsys DC treats
//         the whole always block as one scope: mixing <= and = on the same
//         variable anywhere in the block (even in separate if/else branches)
//         triggers VER-134. Solution: use blocking (=) for Address everywhere.
// -----------------------------------------------------------------------------
always @(posedge CLOCK, posedge RESET) begin
    if(RESET == 1'b1) begin
		BE_n <= 4'b0000;
		Address = 0;           // FIX 1: was <=, must be = to match StateT1/T2P
		W_R_n <= 1'b0;
		D_C_n <= 1'b0;
		M_IO_n <= 1'b0;
		ADS_n <= 1'b0;
